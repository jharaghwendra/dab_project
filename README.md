...
The Gold lineage is dbt-driven: snapshots such as `dim_player`, `dim_tag`, and `dim_userlimit` feed downstream marts, while facts and dimensions are built from the shared Silver layer. Example lineage is `silver.userdata -> snapshot dim_player -> mart_player_activity_summary` and `silver.gameround -> fact_gameround -> mart_gameround_hourly`.
...
| `gold` | `mart_player_activity_summary` | Mart |
